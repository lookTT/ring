...
# 数据库连接 & 全局扫描参数
global_dbcoon = 'mysql+mysqldb://root:@127.0.0.1:3306/ring'
...
global_options = '-sS -Pn --script=banner'
# 处理端口状态
...
def get_db_info():
    index = len('mysql+mysqldb://')
    fast = global_dbcoon.find(':', index)
    dbUser = global_dbcoon[index:fast]

    index += len(dbUser) + 1
    fast = global_dbcoon.find('@', index)
    dbPassWd = global_dbcoon[index:fast]

    index += len(dbPassWd) + 1
    fast = global_dbcoon.find(':', index)
    dbHost = global_dbcoon[index:fast]

    index += len(dbHost) + 1
    fast = global_dbcoon.find('/', index)
    dbPort = global_dbcoon[index:fast]

    index += len(dbPort) + 1
    dbDataBase = global_dbcoon[index:]

    return dbUser, dbPassWd, dbHost, dbPort, dbDataBase
...
def run_wyportmap(targets, taskid=None):
    ...
    # 判断对应二级域名是否已经执行过
    isReturn = False
    dbUser, dbPassWd, dbHost, dbPort, dbDataBase = get_db_info()
    db = MySQLdb.connect(host=dbHost, user=dbUser, passwd=dbPassWd, db=dbDataBase, charset='utf8')
    cur = db.cursor()
    query = "select `port_status` from `pdomain` where `ip`='%s' and `taskid`='%s'" % (targets, taskid)
    try:
        # 执行sql语句
        cur.execute(query)
        # 提交到数据库执行
        ...
        for row in results:
            ...
            if isReturn:
                print 2
                return 'the %s is running or complete' % targets
    except:
        # 返回错误消息
        db.rollback()
        return 'the [%s] is Fail!' % query
